...
library AddressLib {
	...
	function _toChecksumCapsFlags(address account) private pure returns (
    ...
    function _toAsciiString(
		...
		for (uint256 i = 0; i < data.length; i++) {
			...
			asciiBytes[2 * i] = bytes1(leftNibble + (leftNibble < 10 ? 48 : 87))[0];
			asciiBytes[2 * i + 1] = bytes1(rightNibble + (rightNibble < 10 ? 48 : 87))[0];
		}
